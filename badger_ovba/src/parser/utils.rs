...
pub fn peek_u16(cursor: &mut Cursor<&[u8]>) -> Result<u16, Error> {
    ...
    if cursor_pos < internal_ref.len() - 1 {
        let peek_val = u16::from_le_bytes(internal_ref[cursor_pos..cursor_pos+2].try_into().expect("Try into should not fail unless stream is corrupted"));
        Ok(peek_val)
    } else {
        Err(Error::Parser("Parser Error: Index to peek exceeds the length of the slice".into(), cursor_pos as u64))
    }
}
...
pub fn get_u16(cursor: &mut Cursor<&[u8]>) -> Result<u16, Error> {
    ...
    if cursor_pos < internal_ref.len() - 1 {
        ...
        Err(Error::Parser("Parser Error: Index get_u16 exceeds the length of the slice".into(), cursor_pos as u64))
    }
}
...
pub fn get_u32(cursor: &mut Cursor<&[u8]>) -> Result<u32, Error> {
    ...
    if cursor_pos < internal_ref.len() - 3 {
        ...
        Err(Error::Parser("Parser Error: Index for get_u32 exceeds the length of the slice".into(), cursor_pos as u64))
    }
}
...
pub fn get_n_bytes(cursor: &mut Cursor<&[u8]>, n: usize) -> Result<Vec<u8>, Error> {
    ...
    if cursor_pos < internal_ref.len() + n - 1 {
        ...
        Err(Error::Parser("Parser Error: Index for get_n_bytes() exceeds the lenght of the slice".into(), cursor_pos as u64))
    }
}
